...
map_extent='0. 1600 0. 1600'  # (xmin xmax ymin ymax)
run_parallel=1

# Specify directory for the demo data
# There are a total of 4 tests (e.g., test1, test2, test3, test4)
dir_demo='/pscratch/sd/p/paccini/temp/output_tracking/tracking_feature_idealized_rolled_demo/'


# Example config file name
config_demo='/global/cfs/cdirs/wcm_code/lpaccini/PyFLEXTRKR-dev/config/config_feature_pbc_idealized.yml'
...
animation_filename=${animation_dir}feature_tracking_${start_date}_${end_date}.mp4
...
python /global/cfs/cdirs/wcm_code/lpaccini/PyFLEXTRKR-dev/runscripts/run_generic_tracking.py ${config_demo}
...
python /global/cfs/cdirs/wcm_code/lpaccini/PyFLEXTRKR-dev/Analysis/plot_subset_tbpf_tracks_nomap_demo_pbc.py -s ${start_date} -e ${end_date} \
